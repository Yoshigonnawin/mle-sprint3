...
class FastApiHandler:
    ...
    def __init__(self):
        ...
        self.model_path = (
            Path(__file__).parent.parent / "models/catboost_churn_model.bin"
        )
    ...
    def handle(self, params):
        ...
        try:
            if not self.validate_params(params):
                ...
                print(
                    f"Predicting for client_id: {user_id} and model_params:\n{model_params}"
                )
                # получаем предсказания модели
                predicted_proba = self.churn_predict(model_params)
                is_churn = 1 if predicted_proba > 0.5 else 0
                response = {
                    "user_id": user_id,
